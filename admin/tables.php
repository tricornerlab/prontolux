...
if($queryTable = @mysql_query($sqlTable,$db_conn)){
    if(mysql_num_rows($queryTable)>0){
        ...
        $show_table .= '<h4>'.$table_name.'</h4><br /><a href="index.php?ref=del&go=table&id='.$table_id.'&pid='.$prod_id.'" class="delLink">Effacer tableau</a>';
        
        if(strlen($table_file)>0 && file_exists($table_file)){
            $data = new Spreadsheet_Excel_Reader($table_file);
            $show_table .= '<div id="tablaDiv">'.$data->dump(true,true).'</div>';
            
        }elseif(!file_exists($table_file)){
            $show_table .= '<br />Le fichier est introuvable';
        }elseif($table_file != 0){
        ...
        $show_table .= '<a href="index.php?ref=add&go=table&pid='.$prod_id.'" class="addLink">Ajouter tableau</a>';
    }
}
